...
const CATEGORIES = [
  { id: 'Notes', label: 'Clinical Notes', icon: FileText, color: 'text-yellow-600', bg: 'bg-yellow-50', border: 'border-yellow-100' },
  { id: 'Photographs', label: 'Photographs', icon: ImageIcon, color: 'text-purple-600', bg: 'bg-purple-50', border: 'border-purple-100' },
  { id: 'Reports', label: 'Lab Reports', icon: FileBarChart, color: 'text-blue-600', bg: 'bg-blue-50', border: 'border-blue-100' },
  { id: 'Scans', label: 'X-Rays & Scans', icon: HardDrive, color: 'text-slate-600', bg: 'bg-slate-50', border: 'border-slate-100' },
];

const ReportsNotesSection = ({ patientId }) => {
  const [view, setView] = useState('DASHBOARD'); 
  const [activeCategory, setActiveCategory] = useState(null);
  
  // Real Data State
  const [data, setData] = useState({ Notes: [], Photographs: [], Reports: [], Scans: [] });
  ...
  const fetchFiles = async () => {
    try {
      setLoading(true);
      // Assuming your backend returns an array of file objects for this patient
      const res = await API.get(`/files/patient/${patientId}`);
      
      // Group flat array into categories map
      const groupedData = { Notes: [], Photographs: [], Reports: [], Scans: [] };
      res.data.forEach(file => {
        if (groupedData[file.category]) {
          groupedData[file.category].push(file);
        }
      });
      setData(groupedData);
    } catch (err) {
  ...
  const getCategoryStats = (catId) => {
    ...
    return { count: files.length, lastUpdated: new Date(sorted[0].date).toLocaleDateString() };
  };
  ...
  const renderDashboard = () => (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 animate-in fade-in duration-300">
      {CATEGORIES.map((cat) => {
        ...
        return (
          <div key={cat.id} onClick={() => { setActiveCategory(cat.id); setView('LIST'); }} className={`p-5 rounded-xl border ${cat.border} ${cat.bg} cursor-pointer transition-all hover:shadow-md flex flex-col justify-between h-32 group`}>
    ...
    return (
      <div className="animate-in slide-in-from-right-4 duration-300">
        <div className="flex items-center gap-3 mb-4 pb-2 border-b border-gray-100">
           <button onClick={() => setView(mode === 'VIEW' ? 'DASHBOARD' : 'PREVIEW')} className="p-1.5 hover:bg-gray-100 rounded-full text-gray-500 transition-colors">
           ...
           <div>
             <h3 className="font-bold text-md text-slate-800">{mode === 'COMPARE' ? 'Select File to Compare' : category.label}</h3>
        ...
        {files.length > 0 ? (
          <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3">
            {sortedFiles.map(file => (
              <div key={file._id} onClick={() => handleFileClick(file)} className={`flex items-center gap-3 p-3 bg-white border rounded-lg hover:shadow-md hover:border-[#137fec] transition-all cursor-pointer ${selectedFile?._id === file._id && mode === 'COMPARE' ? 'opacity-50 pointer-events-none bg-gray-50' : ''}`}>
                <div className={`w-10 h-10 rounded-lg flex items-center justify-center shrink-0 ${category.bg} ${category.color}`}>
                   {file.type === 'image' || file.type === 'xray' ? <ImageIcon size={18} /> : <FileText size={18} />}
                </div>
                <div className="min-w-0">
                   <h5 className="font-semibold text-xs text-slate-800 truncate leading-tight mb-1" title={file.name}>{file.name}</h5>
                   <div className="flex items-center gap-1 text-[10px] text-slate-400">
                      <Calendar size={10} /> {new Date(file.date).toLocaleDateString()}
                   </div>
                </div>
              </div>
            ))}
    ...
    <div className="animate-in zoom-in-95 duration-200 h-[500px] flex flex-col">
       <div className="flex justify-between items-center mb-4 border-b border-gray-100 pb-3">
          <div className="flex items-center gap-3">
             <button onClick={() => setView('LIST')} className="flex items-center gap-1 text-sm text-slate-500 hover:text-slate-800"><ChevronLeft size={16}/> Back</button>
             <h3 className="font-bold text-lg text-slate-800">{selectedFile.name}</h3>
          </div>
          <button onClick={() => setView('COMPARE_SELECT')} className="flex items-center gap-2 px-4 py-2 bg-slate-900 text-white rounded-lg hover:bg-slate-700 transition-colors text-sm font-bold shadow-md">
       ...
       <div className="flex-1 bg-slate-100 rounded-xl flex items-center justify-center overflow-hidden border border-slate-200">
          {selectedFile.url ? (
             <img src={selectedFile.url} alt="Preview" className="max-h-full max-w-full object-contain shadow-lg" />
          ) : (
    ...
    <div className="fixed inset-0 z-[300] bg-slate-900 flex flex-col animate-in fade-in duration-300">
       ...
       <div className="flex-1 flex overflow-hidden">
          <div className="flex-1 bg-black/90 flex flex-col border-r border-slate-700">
             <div className="p-2 bg-black text-white text-xs text-center opacity-70 border-b border-slate-800">{selectedFile?.name}</div>
             <div className="flex-1 flex items-center justify-center p-4">
                {selectedFile?.url && <img src={selectedFile.url} className="max-w-full max-h-full object-contain" />}
             </div>
          </div>
          <div className="flex-1 bg-black/90 flex flex-col">
             <div className="p-2 bg-black text-white text-xs text-center opacity-70 border-b border-slate-800">{compareFile?.name}</div>
             <div className="flex-1 flex items-center justify-center p-4">
                {compareFile?.url && <img src={compareFile.url} className="max-w-full max-h-full object-contain" />}
             </div>
          </div>
       </div>
    </div>
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center bg-black/40 backdrop-blur-sm p-4">
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl p-6">
        ...
        <form onSubmit={handleUpload} className="flex flex-col gap-4">
          ...
          <button disabled={uploading} type="submit" className="w-full bg-[#137fec] text-white font-bold py-3 rounded-lg hover:bg-blue-600 transition-colors mt-2 flex items-center justify-center gap-2">
  ...
  return (
    <div className="p-5 bg-white rounded-xl shadow-sm border border-gray-100 mb-6 relative">
      <div className="flex justify-between items-center mb-6">
         ...
         <button onClick={() => setIsUploadOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-[#137fec] text-white rounded-lg hover:bg-blue-600 transition-colors font-medium text-sm shadow-sm">
            <Plus size={18} /> Add File
         </button>
      </div>
      
      {loading ? (
        <div className="h-32 flex items-center justify-center"><Loader2 className="animate-spin text-[#137fec]" size={32}/></div>
      ) : (
        <>
