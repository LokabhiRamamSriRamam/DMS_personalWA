...
const InventoryStocks = ({ StockBadge, SectionHeader, searchQuery, isLowStock, medicineEnabled = true, consumableEnabled = true }) => {
  ...
  const Section = ({ title, icon: Icon, colorClass, data, expanded, setExpanded, usageLabel }) => (
      ...
      <div className={`hidden sm:grid grid-cols-1 ${medicineEnabled && consumableEnabled ? 'lg:grid-cols-2' : ''} gap-6 h-full min-h-0`}>
        {medicineEnabled && (
          <div className="flex flex-col bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden h-full">
            <button onClick={() => setPharmacyExpanded(v => !v)} className="flex items-center justify-between w-full text-left">
        ...
        {consumableEnabled && (
          <div className="flex flex-col bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden h-full">
            <button onClick={() => setConsumablesExpanded(v => !v)} className="flex items-center justify-between w-full text-left">
